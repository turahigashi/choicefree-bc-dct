...
Version: 0.7.8  ·  Toshihisa Urahigashi ([ORCID 0009-0004-0460-6242](https://orcid.org/0009-0004-0460-6242))

DOI (this version): [10.5281/zenodo.22333837](https://doi.org/10.5281/zenodo.22333837)  ·  previous version v0.7.7: [10.5281/zenodo.22330020](https://doi.org/10.5281/zenodo.22330020)  ·  earlier versions via the concept DOI
...
The overall marker is written only after every stage has passed and after
`tools/check_log_generation.py --pending` has inspected the log this run just wrote, so a
run that fails anywhere leaves no marker at all.  Two earlier arrangements were wrong and a
reviewer reproduced both: writing the marker before the last stages let a failing run ship a
success line, and moving the check after the marker meant no run could ever write it.

Checking the *stored* reference log is a separate operation and is not part of this script.
It is a release gate, run once when a deposit is cut:

```bash
python3 tools/check_log_generation.py     # the stored logs/build_audit.txt
python3 tools/check_public_state.py       # tag, deposit and working tree agree
python3 tools/check_submission_pdf.py paper/paper.tex paper/paper.pdf
```

The last of these compares the release identity a compiled manuscript *displays* --- its
cover, its self-describing passages and its link annotations --- with the TeX it was built
from.  Every other check reads the TeX, and the root checksums exclude `paper/`, so a stale
PDF passed all of them: v0.7.8 bumped the version everywhere and rebuilt only the title page,
and the manuscript still announced the previous version and DOI.  A reviewer found it; the
check is theirs, adopted here.

Keeping the stored-log check inside `build_audit.sh` forced a choice between two wrong
things: fail before the marker, and no promoted log can ever satisfy it, so no release is
reachable; fail after it, and a failing run ships a log that says it succeeded.
...
`CITATION.cff` contains the author and DOI metadata for this release.  The concept DOI `10.5281/zenodo.21850965` resolves to the latest version; the v0.7.8 deposit carries the version DOI `10.5281/zenodo.22333837`.
