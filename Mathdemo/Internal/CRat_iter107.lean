import Mathdemo.Internal.CRat_iter106

/-!
# COFOC assembly from global reps and a PosEventually selector

`CRat_iter106` exposed the remaining positivity bridge as a selector for the
`k, N` witnesses inside `PosEventually`.  This file threads that selector
through the localized positive-order-data branch from `CRat_iter97` through
`CRat_iter99`.
