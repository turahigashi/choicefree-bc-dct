import Mathdemo.Internal.CRat_iter13
...
/-- Audited raw subtraction seed. -/
structure CRealSubSeed : Type where
  sub_regular : ∀ x y : RegularSeq, RegularVal (subVal x.val y.val)
  sub_respects : ∀ x x' y y' : RegularSeq,
    rel x x' → rel y y' → relVal (subVal x.val y.val) (subVal x'.val y'.val)

def cRealSubSeed : CRealSubSeed where
  sub_regular := sub_regular
  sub_respects := sub_respects
