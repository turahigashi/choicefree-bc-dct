import Mathdemo.Internal.Real.RegularityDiagonalRepresentativeCandidate

/-!
# Representative diagonal limit close data

`RegularityDiagonalRepresentativeCandidate` proved that the diagonal candidate is a regular representative.
This file proves the remaining tail-closeness estimate and bundles the actual
`CRealRepDiagonalLimitCloseData` required by `RepresentativeDiagonalClosenessQuotientConvergence`.
-/
...
/-- The representative diagonal-limit close data required by `RepresentativeDiagonalClosenessQuotientConvergence`. -/
