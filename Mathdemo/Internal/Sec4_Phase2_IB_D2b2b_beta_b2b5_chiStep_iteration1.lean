...
theorem sec4_chiStep_coverDiff
    ...
    (hV : RSeq.SeriesSum
      (fun m => COF.abs ((((sec4CoverAnd_int B hB f k).rep.fn m).toFun x))))
    (hD : RSeq.SeriesSum
      (fun m => COF.abs ((((sec4CoverDiff_int B hB f k).rep.fn m).toFun x))))
    (hU : RSeq.SeriesSum
      (fun m => COF.abs ((((sec4CoverAnd_int B hB f (k + 1)).rep.fn m).toFun x)))) :
    (seriesSum_of_abs hV).sum + (seriesSum_of_abs hD).sum =
      (seriesSum_of_abs hU).sum := by
  have vV := (sec4CoverAnd_int B hB f k).valid x hV
  have vD := (sec4CoverDiff_int B hB f k).valid x hD
  have vU := (sec4CoverAnd_int B hB f (k + 1)).valid x hU
  cases vV.1 with
  | inl hV1 =>
...
theorem sec4_chi_telescope_s1_of_atomData
    ...
    ∀ x : X, x ∈ B.S1 →
      ...
      ∀ n : Nat,
        (seriesSum_of_abs (T.base_chi_abs x hgenabs)).sum +
          RSeq.partialSum
            (fun k => (seriesSum_of_abs (T.term_chi_abs k x hgenabs)).sum) n =
        (seriesSum_of_abs (T.cover_chi_abs_succ x hgenabs n)).sum := by
  intro x hxB hgenabs
  let base : R := (seriesSum_of_abs (T.base_chi_abs x hgenabs)).sum
  ...
  · exact sec4_chiStep_coverDiff B hB f 0 x
      (T.base_chi_abs x hgenabs)
      (T.term_chi_abs 0 x hgenabs)
      (T.cover_chi_abs_succ x hgenabs 0)
  · intro k
    exact sec4_chiStep_coverDiff B hB f (k + 1) x
      (T.cover_chi_abs_succ x hgenabs k)
      (T.term_chi_abs (k + 1) x hgenabs)
      (T.cover_chi_abs_succ x hgenabs (k + 1))
