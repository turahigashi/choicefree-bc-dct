import Mathdemo.Internal.CRat_iter105

/-!
# PosEventually witness selector frontier

`CRat_iter105` reduced Prop-valued quotient order extraction to two separate
ingredients:
...
that selector back into the `CRat_iter105` route.
-/
...
positivity bridge isolated in `CRat_iter105`. -/
...
/-- The `CRat_iter105` quotient-order extraction route restated with the
