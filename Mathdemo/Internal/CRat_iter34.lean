import Mathdemo.Internal.CRat_iter33

/-!
# CReal quotient multiplication from the Phase 10 closure

`CRat_iter28` exposed quotient multiplication conditionally on
`CRealMulClosureData`.  `CRat_iter33` constructs that closure data from the
...
/-- Phase 10 discharges the completion obligations recorded in `CRat_iter29`. -/
...
/-- The conditional quotient multiplication seed from `CRat_iter28`, now
specialized with the closure constructed in `CRat_iter33`. -/
